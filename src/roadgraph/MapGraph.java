...
public class MapGraph {
	...
	public void printEdges() {
		...
		for (MapEdge e : edges) {
			...
			if(!types.contains(e.getRoadType())){
				types.add(e.getRoadType());
			}
		}
	...
	public List<GeographicPoint> dijkstra(GeographicPoint start, GeographicPoint goal,
		...
		// set all vertices with infinity distance
		Collection<GeographicPoint> vertices = this.getVertices();
		for (GeographicPoint vertex : vertices) {

			MapNode node = pointNodeMap.get(vertex);
			node.setDistance(Double.POSITIVE_INFINITY);

		}

		// Enqueue start node into Priority Queue with distance 0
		...
		while (!pq.isEmpty()) {
			...
			if (!visited.contains(curr)) {
				...
				for (MapEdge edge : curr.getEdges()) {
					...
					MapNode n = edge.getOtherNode(curr);
					
					//if not visited
					if (!visited.contains(n)) {
						
						// path between curr to n
						double distance = curr.getDistance() + edge.getLength();
						
						// if path though curr to n is shorter
						if (distance <= n.getDistance()) {
							//update n's distance
							n.setDistance(distance);
							//update curr as n's parent 
							parentMap.put(n, curr);							
							//enqueuq n, distance in pq
							pq.add(n);
						}
					}
				}
	...
	public List<GeographicPoint> aStarDistanceSearch(GeographicPoint start, GeographicPoint goal) {
		// Dummy variable for calling the search algorithms
		Consumer<GeographicPoint> temp = (x) -> {
		};
		return aStarSearch(start, goal, temp);
		...
		while (!pq.isEmpty()) {
			...
			if (!visited.contains(curr)) {
				...
				for (MapEdge edge : curr.getEdges()) {
					...
					MapNode n = edge.getOtherNode(curr);
				    //calculate linear distance between nodes for A star
					...
					if (!visited.contains(n)) {
						...
						// path between curr to n
						double distance = curr.getDistance() + edge.getLength();
						double predictedDistance  = distance + n.getLocation().distance(goal);
						

						
						// if path though curr to n is shorter
						if (predictedDistance <= n.getActualDistance()) {
							//update n's distance
							n.setDistance(distance);
							n.setActualDistance(predictedDistance);
							//update curr as n's parent 
							parentMap.put(n, curr);							
							//enqueuq n, distance in pq
							pq.add(n);
	...
	public List<GeographicPoint> aStarDistanceSearch(GeographicPoint start, GeographicPoint goal,
		...
		for (GeographicPoint vertex : vertices) {
			...
			MapNode node = pointNodeMap.get(vertex);
		//	node.setDistance(Double.POSITIVE_INFINITY);
			node.setArrivalTime(Double.POSITIVE_INFINITY);
			//node.setActualDistance(node.getLocation().distance(goal));
			//node.setActualDistance(Double.POSITIVE_INFINITY);
			node.setActualArrivalTime(Double.POSITIVE_INFINITY);

		}

		// Enqueue start node into Priority Queue with distance 0
	//	startNode.setDistance(0);
		startNode.setArrivalTime(0);
		pq.add(startNode);
			...
			if (!visited.contains(curr)) {
				...
				for (MapEdge edge : curr.getEdges()) {
					...
					//	double predictedDistance  = distance + n.getLocation().distance(goal);
						...
						double arrivalTime = curr.getArrivalTime()+(edge.getLength()/edge.getSpeedLimit());
						double predictedArrivalTime = arrivalTime + (n.getLocation().distance(goal)/50);
						

						
						// if path though curr to n is shorter
						if (predictedArrivalTime <= n.getActualArrivalTime()) {
							//update n's distance
							//n.setDistance(arrivalTime);
							//n.setActualDistance(predictedArrivalTime);
							
							n.setArrivalTime(arrivalTime);
							n.setActualArrivalTime(predictedArrivalTime);
							//update curr as n's parent 
							parentMap.put(n, curr);							
							//enqueuq n, distance in pq
							pq.add(n);
	...
	public static void main(String[] args) {
		...
		List<GeographicPoint> route = theMap.dijkstra(new GeographicPoint(1.0,
		 1.0), new GeographicPoint(8.0, -1.0));
		
		List<GeographicPoint> route2 = theMap.aStarSearch(new GeographicPoint(1.0,
				 1.0), new GeographicPoint(8.0, -1.0));
		...
		//	GraphLoader.loadRoadMap("data/maps/gdl.map", theMap);
